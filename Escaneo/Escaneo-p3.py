...
def escanearhost(host,puerto):
	...
	print('\n')
	print(' Resultado del escaneo de {}'.format(host))
	print('----------------------------------------')
	for p in puerto:
		...
		if s.connect_ex((host,int(p))):
			...
			print('Puerto {} Abierto - '.format(p),s.recv(1024).decode('ISO-8859-1'))
		s.close()
				...
				for z in range(1,255):
					...
					for p in puerto:
						...
						if s.connect_ex((host,int(p))):
							...
							print('Puerto {} Abierto - '.format(p),s.recv(1024).decode('utf-8'))
						s.close()
...
def main():
	op = opcioneshost()
	if op == 1:
		host = str(input('Indica IP del host: '))
		puerto = opcionespuerto()
		escanearhost(host,puerto)
	if op == 2:
		red = str(input('Indica IP de la red <IP>/<Mascara>: '))
		lista = red.split('/')
		while(len(lista) != 2):
			print('Error de sintaxis')
			red = str(input('Vuelva a introducir IP de la red <IP>/<Mascara>: '))
			lista = red.split('/')
		ip,tipo = ipred(lista)
		puerto = opcionespuerto()
